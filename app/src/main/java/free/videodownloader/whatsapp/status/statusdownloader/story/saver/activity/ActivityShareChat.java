...
import free.videodownloader.whatsapp.status.statusdownloader.story.saver.databinding.ActivitySharechatBinding;
...
import static free.videodownloader.whatsapp.status.statusdownloader.story.saver.utils.Util.RootDirectoryShareChat;
...
public class ActivityShareChat extends AppCompatActivity {
    private ActivitySharechatBinding binding;
    ActivityShareChat activity;
...
//        MyAds.showGoogleSmartBanner(this, findViewById(R.id.banner_container_smart));
    ...
    private void initViews() {
        ...
        if (!SharePrefereces.getInstance(activity).getBoolean(SharePrefereces.ISSHOWHOWTOSHARECHAT)) {
            SharePrefereces.getInstance(activity).putBoolean(SharePrefereces.ISSHOWHOWTOSHARECHAT, true);
            binding.layoutHowTo.LLHowToLayout.setVisibility(View.VISIBLE);
        } else {
        ...
        binding.LLOpenTwitter.setOnClickListener(v -> {
            Util.OpenApp(activity, "in.mohalla.sharechat");
        });
    }
