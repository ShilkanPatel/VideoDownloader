...
import static free.videodownloader.whatsapp.status.statusdownloader.story.saver.utils.Util.RootDirectorySnackVideoShow;

public class FragmentSnackVideoDownloaded extends Fragment implements FileListClickInterface {
    private FragmentHistoryBinding binding;
    private RvFileListAdapter rvFileListAdapter;
    private ArrayList<File> fileArrayList;
    private ActivityGallery activity;
    public static FragmentSnackVideoDownloaded newInstance(String param1) {
        FragmentSnackVideoDownloaded fragment = new FragmentSnackVideoDownloaded();
    ...
    private void getAllFiles(){
        ...
        File[] files = RootDirectorySnackVideoShow.listFiles();
        if (files!=null) {
            for (File file : files) {
                fileArrayList.add(file);
            }
            rvFileListAdapter = new RvFileListAdapter(activity, fileArrayList, FragmentSnackVideoDownloaded.this);
